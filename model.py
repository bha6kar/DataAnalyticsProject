...
class NeuralNetwork:
    ...
    def make_train_model(self, epochs=1000):
        ...
        layer = kl.Dense(
            16, activation="sigmoid", activity_regularizer=regularizers.l2(0.005)
        )(lstm)
        lstm2 = kl.LSTM(
            8,
            activity_regularizer=regularizers.l2(0.01),
        ...
        out = kl.Dense(
            1, activation="sigmoid", activity_regularizer=regularizers.l2(0.001)
        )(lstm2)
        ...
        train = np.reshape(
            np.array(pd.read_csv("features/autoencoded_train_data.csv", index_col=0)),
            (
                len(np.array(pd.read_csv("features/autoencoded_train_data.csv"))),
                1,
                self.input_shape,
            ),
        )
        ...
        test_x = np.reshape(
            np.array(pd.read_csv("features/autoencoded_test_data.csv", index_col=0)),
            (
                len(np.array(pd.read_csv("features/autoencoded_test_data.csv"))),
                1,
                self.input_shape,
            ),
        )
        ...
        predicted_data[0].plot(
            label="Predicted Price", figsize=(16, 8), title="Prediction vs Actual"
        )
